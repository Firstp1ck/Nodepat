    ...
    egui::Window::new("Font")
        ...
        .show(ctx, |ui| {
            ui.vertical(|ui| {
                ...
                ui.label("Size:");
                ui.add(egui::Slider::new(&mut app.format_settings.font_size, 8.0..=72.0));
            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Go To").clicked() {
                        if let Ok(line_num) = app.goto_line.parse::<usize>() {
...
fn show_save_dialog(ctx: &egui::Context, app: &mut NodepatApp) {
        ...
        .save_file()
    {
        if let Some(path_str) = path.to_str() {
            if let Err(e) = app
                .file_state
                .save_file(path_str, &app.editor_state.text)
            {
